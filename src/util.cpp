...
#else
    const char* pszModule = "smartiecoinclassic";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Smartiecoinclassic";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Smartiecoinclassic";
#else
    // Unix
